...
global  {
	...
	float last_action_time <- gama.machine_time;
	...
	reflex detect_interaction_discussion_phase when: stage = PLAYER_DISCUSSION_TURN {
		...
		if ready_action and gama.machine_time > (last_action_time + (1000.0 * 2 * delay_between_actions)) {
			latest_action <- "";
		}
		if result != latest_action and result = A_END_TURN {
			...
			last_action_time <- gama.machine_time;
	...
	reflex detect_interaction when: stage = PLAYER_ACTION_TURN and (gama.machine_time > (last_action_time + (1000.0 * delay_between_actions))){
		string result <- string(decodeQR(cam,image_width::image_height, false));
		if result = nil {
			ready_action <- true;
		}
