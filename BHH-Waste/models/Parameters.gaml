...
global {
	...
	float rate_diffusion_solid_waste <- 1.25;//rate of solid waste per perimeter of canals passing to the the downstream canal
	...
	float ground_solid_waste_pollution_impact_rate <- 0.07; //production (yield) = production  - (sum solid pollution on cell * pollution_impact_rate)
	...
	float quantity_communal_landfill_to_treatment <- 800.0; //quantity of solid waste "treated" (that disapears) every day from the communal landfill
	...
	float collection_team_collection_capacity_day <- 875.0; //quantity of solid waste remove during 1 day of work
